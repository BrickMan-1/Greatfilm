var TI =
  '<img src="{src}" alt="" style="display: block;width: 120px;height: auto;margin: 0 10px 0 0;padding: 0;">';
var TPL = [
...
Map.prototype = $.extend({}, Map.prototype, {
  init: function() {
    ...
    // this.initMarkers(this.ops.markers); //用于多个定位图标同时显示
    this.addMarkers(this.ops.marker); //用于单个定位图标显示
  ...
  initMarkers: function(markers) {
    ...
    markers.forEach((item, index) => {
      ...
      _createIcon(this.ops.markIconSrc, function(c) {
        var a = item.point.split(","),
          p = new BMap.Point(a[0], a[1]),
          m = new BMap.Marker(p, {
            icon: c
          }),
          w = _getInfoWindow(item.title, item.summary, item.tel);
        self.bMap.addOverlay(m);
        item.tips && m.setLabel(_getTips(item.tips, c));
        m.addEventListener(
          "click",
          function() {
  ...
  addControl: function(anchor, type) {
    ...
    switch (anchor) {
      case 2:
        a = BMAP_ANCHOR_TOP_RIGHT;
        break;
      case 3:
        a = BMAP_ANCHOR_BOTTOM_LEFT;
        break;
      case 4:
        a = BMAP_ANCHOR_BOTTOM_RIGHT;
        break;
      default:
        a = BMAP_ANCHOR_TOP_LEFT;
    }
    switch (type) {
      case 2:
        t = BMAP_NAVIGATION_CONTROL_SMALL;
        break;
      case 3:
        t = BMAP_NAVIGATION_CONTROL_PAN;
        break;
      case 4:
        t = BMAP_NAVIGATION_CONTROL_ZOOM;
        break;
      default:
        t = BMAP_NAVIGATION_CONTROL_LARGE;
    }
    controler = new BMap.NavigationControl({
      anchor: a,
      type: t
    });
