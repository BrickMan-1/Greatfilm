...
export default function API(options) {
    var defaults = {
        ...
        decode: ['zhaiyao', 'content', 'description'],
        pathToAbsolute: true
    };
...
function ajax(options) {
    ...
    function readyStateChangeFn() {
        ...
        if (options.dataType === 'json') {
            try {
                // console.log('p2a: %o', p2a);
                response = JSON.parse(xhr.responseText);
                // p2a && path2Absolute(response, basePath);
...
function megerOptions(api, fnName, options) {
    return extend({
        ...
        pathToAbsolute: api.options.pathToAbsolute
    }, options || {});
}
